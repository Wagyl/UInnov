...
Nous allons donc considérer une application gérant une liste et un
compteur. Elle est munie d'une interface graphique constituée d'une
fenêtre, ainsi que de deux vues, ou paneaux, qui ne sont pas visibles
en même temps, mais avec la possibilité de passer de l'une à l'autre.
Ces vues seront affichées, lorsque visibles, dans la fenêtre précédement
mentionnée. 

L'une des deux vues offrira une représentation et des outils de modification
de la liste gérée par l'application alors que l'autre se chargera
du compteur. 

Deux menus sont présents, l'un attaché à la fenêtre et l'autre \og flottant \fg{}
accessible via une intéraction entre l'utilisateur et l'interface.
...
pas forcement être exhaustifs, mais nous allons essayer de donner
une bonne idée de ce que nous aurons à faire.

Nous allons commencer par étudier les besoins de l'utilisateur application.
\begin{itemize}
\item \textbf{Annulation :} pouvoir annuler une opération effectuée via
l'interface graphique sur la liste ou le compteur.
...
un smartphone. Il en est de même pour toutes les actions proposées
par l'interface, y compris la gestion du menu flottant (apparition
au clic droit pour le clavier / souris et utilisation d'une \og touche \fg{}
...
l'utilisateur qu'il est en attente passive. Les opérations sur la
liste et le compteur peuvent être soumises à des contraintes au niveau
de la partie logique de l'application, l'interface doit être capable
d'afficher une barre de progression par exemple pour montrer qu'une
action est en cours.
\end{itemize}
Ceux du programmeur en découlent en partie :
\begin{itemize}
...
l'implémenter serait pratique.
\item \textbf{Adaptativité : }le programmeur ne doit pas avoir à programmer
des interfaces différentes juste pour changer un bouton en \og gesture \fg{}.
Il doit en être de même pour les autres actions comme l'annulation
ainsi que l'aspect graphique général.
\end{itemize}
...
Exemple suivi : (si le programmeur veut satisfaire aux besoins d'aptabilité
de l'utilisateur application, il va être obligé de définir plusieurs
interfaces différentes, pour les actions, faire en sorte que les raccourcis
dans le menu, les boutons ou autres gestures soient associés à des
actions est possible en swing, mais assez moche et peu pratique, de
plus elles ne sont pas assez évoluées pour permettre une annulation
par la suite. La mise à jour de la liste de du compteur ne sont pas
gérés de façon pratique par SWING, le faire manuellement est finalement
