...
La multiplication récente des supports multimédia a mené à une évolution
et une diversification des modes d'intéraction entre l'homme et la
machine. La conception d'interfaces utilisateurs, en particulier d'interfaces
graphiques, n'en est devenue que plus complexe mais surtout différente.
La multiplicité des supports force les développeurs à adapter leurs
produits aux différents écosytèmes faisant ainsi naître de nouveaux
besoins en terme d'outils de création d'interfaces.

Le rapport qui suit est le fruit d'une réflexion que nous avons menée
sur cette nouvelle approche des interfaces utilisateurs. Nous avons
concentré notre attention sur les points qui nous paraissaient important,
laissant ainsi de côté certains aspects techniques comme l'utilisation
de la concurrence pour profiter de l'accroissement du nombre de processeurs
dans les machines ou la gestion des affichages via les bibliothèques
système.

Pour illustrer notre propos, nous allons introduire un exemple simple
qui sera utilisé tout au long du rapport.\\
...
Ce dont nous ne nous occuperons pas dans ce rapport : une interface
graphique n'est qu'un intermédiaire entre une application et ses utilisateurs
finaux ; nous considérons donc que la conception de l'interface doit
pouvoir se greffer par dessus un programme pré-existant selon le pattern
modèle-vue-contrôleur qui tend à minimiser les dépendances entre chacune
des parties. Une bibliothèque graphique n'a donc pas à se charger
du développement de la logique interne de l'application, mais doit
permettre d'afficher sous diverses formes des informations multiples
provenant du modèle, et de transmettre des actions de l'utilisateur
au contrôleur.

Nous ferons désormais référence à cette \og application sous-jacente \fg{}
comme du moteur ou de la logique interne de l'application. Notons
qu'une bibliothèque graphique doit permettre de construire une interface
pour des programmes les plus variés possible.
...
\medskip{}
\begin{figure}[H]
\centering{}\includegraphics[scale=0.5]{images/modele-relationnel-projet}\caption{Concept relationnel}
\end{figure}
~\\
...
L'important est que la description soit proche du modèle et ce afin
de facilité le passage de la réflexion au concrêt, on remarque de
plus que l'adaptation de l'interface au support est déjà importante
dans un exemple simple comme celui-ci. La présence ou absence d'une
souris est un élément prépondérant dans le choix de l'outil de transition
entre les deux vues.
\end{document}
